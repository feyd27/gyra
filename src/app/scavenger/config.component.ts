...
export class ConfigComponent implements OnInit {
  ...
  pools = [
   ...
   // new PoolPicker (3, 'PoCC Pool 50-50', 'http://50-50-pool.burst.cryptoguru.org:8124', 31536000),
  // new PoolPicker (4, 'PoCC Pool 100-0', 'http://100-0-pool.burst.cryptoguru.org:8124', 86400),
    new PoolPicker (2, 'VoipLanParty Pool 0-100', 'http://voiplanparty.com:8124', 31536000),
    new PoolPicker (3, 'Pool Burstcoin Space 0-100', 'http://pool.burstcoin.space:8124', 2592000),
    new PoolPicker (4, 'Pool.Burstcoin.RO 50-50', 'http://pool.burstcoin.ro:8080', 31536000),
    new PoolPicker (5, '0-100Pool.Burstcoin.RO 0-100', 'http://0-100pool.burstcoin.ro:8880', 16711252963),
    new PoolPicker (6, 'Foxy Pool 0-100', 'https://burst.foxypool.io/', 31536000),
    new PoolPicker (7, 'Burst Neon 50-50', 'http://burstneon.com:2082', 63072000),
    new PoolPicker (8, 'Burst Neon 0-100', 'http://burstneon.com:2052', 63113904),
    new PoolPicker (9, 'Burst Neon 20-80', 'http://burstneon.com:8080', 63113904),
    ...
    new PoolPicker (10, 'BMF Pool (0-100), configurable', 'http://Pool.BurstCoin.ro:8080', 1000000000),
    ];
...
exportConfig(): void {
  ...
  // miner settings
  const hddReaderThreadCount = this.miner_settings.get('hdd_reader_thread_count').value;
  configYaml += 'hdd_reader_thread_count: ' + hddReaderThreadCount;
  const hddUseDirectIO = this.miner_settings.get('hdd_use_direct_io').value;
  configYaml += '\r\nhdd_use_direct_io: ' + hddUseDirectIO.toString();
  const hddWakeUpAfter = this.miner_settings.get('hdd_wake_up_after').value;
  configYaml += '\r\nhdd_wakeup_after: ' + hddWakeUpAfter.toString() + '\r\n';
  // mining settings
  const targetDeadline = this.configForm.get('target_deadline').value;
